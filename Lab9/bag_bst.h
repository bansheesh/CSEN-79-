...
// to a copy of the removed item.
{
  ...
  if (root_ptr->right() != NULL)
    ...
    // STUDENT WORK
    removed = root_ptr->data(); //copy removed data
    oldroot_ptr = root_ptr; 
    root_ptr = root_ptr->left(); //update root_ptr
    delete oldroot_ptr;
  }
  return;
}
...
// function returns false).
{
  ...
  if (target < root_ptr->data()) { // Continue looking in the left subtree
    // STUDENT WORK
    return bst_remove(root_ptr->left(), target); //recursion to search left
  }

  if (target > root_ptr->data()) { // Continue looking in the right subtree
    // STUDENT WORK
    return bst_remove(root_ptr->right(), target); //recursion to search right
  }

  // Target found
  if (root_ptr->left() == NULL) { // Target was found and there is no left subtree, so we can
    // remove this node, making the right child be the new root.

    // STUDENT WORK
    oldroot_ptr = root_ptr; 
    root_ptr = root_ptr->right(); //update root_ptr
    delete oldroot_ptr; //removes the old root_ptr
    return true;
  }
  ...
  // STUDENT WORK
  bst_remove_max(root_ptr->left(), root_ptr->data()); //removes the max value from the left subtree
  return true;
}

template <class Item>
typename bag<Item>::size_type bst_remove_all(binary_tree_node<Item> *&root_ptr,
                                             const Item &target)
// Precondition: root_ptr is a root pointer of a binary search tree
// or may be NULL for an empty tree).
...
// of the target were removed.
{
  ...
  if (target < root_ptr->data()) { // Continue looking in the left subtree
    ...
    // STUDENT WORK
    bst_remove_all(root_ptr->left(), target); //recusive call to search left
  }

  if (target > root_ptr->data()) { // Continue looking in the right subtree

    // STUDENT WORK
    bst_remove_all(root_ptr->right(), target);//recusive call to search right
  }
  ...
  oldroot_ptr = root_ptr;
  bst_remove_max(root_ptr->left(), root_ptr->data()); //removes the max value from the left subtree
  return 1 + bst_remove_all(root_ptr, target); //recursion to search for more copies of the target
}
...
//    • We stop the search just before the cursor falls off the bottom of the
//    tree,
//      and we insert the new entry at the spot where the cursor was about to
//      fall off
template <class Item>
...
// Header file used: bintree.h
{
  ...
  do {
    if (cursor->data() >= entry) { // Go left

      // STUDENT WORK
      if (cursor->left() == NULL) { //create and insert new left node
      ...
      // STUDENT WORK
      if (cursor->right() == NULL) { //create and insert new right node
...
typename bag<Item>::size_type bag<Item>::count(const Item &target) const {
  ...
  while (cursor != NULL) {
    if (cursor->data() < target)
      ...
      // STUDENT WORK
      if (target == cursor->data()) //if target is found, increment answer
        answer++;
      // go left
      cursor = cursor->left();
    }
...
// The assignment operator:
// 1. First check if it is a self-assignment by comparing (this == &source): If
// yes, then return
// 2. If there is no self-assignment, then before we copy the source tree we
// must release all
...
// Header file used: bintree.h
{
  ...
  if (root_ptr == source.root_ptr)
    return;
  tree_clear(root_ptr); //clears the current tree
  root_ptr = tree_copy(source.root_ptr); //copies the source tree
}
...
template <class Item> void bag<Item>::operator+=(const bag<Item> &addend) {
  if (root_ptr == addend.root_ptr) {
    ...
    insert_all(addend.root_ptr); //inserts all the nodes from the addend tree into the current tree
}
...
bag<Item> operator+(const bag<Item> &b1, const bag<Item> &b2) {
  ...
  temp += b1;
  temp += b2;
  return temp; //return the union of b1 and b2
}
...
// method.
{
  if (addroot_ptr != NULL) {
    // STUDENT WORK
    insert(addroot_ptr->data()); //inserts the data from the addroot_ptr
    insert_all(addroot_ptr->left()); //recursion to insert all the left nodes
    insert_all(addroot_ptr->right()); //recursion to insert all the right nodes
  }
}
  }
}
