...
//     deque::BLOCKPOINTER_ARRAY_SIZE is the number of enteries in the array of
//     block pointers
//
//   typedef ____ Item
//     sequence::Item is the data type of the items in the deque. It may be any
...
//     Postcondition: The deque contains an array of block pointers with
//     BLOCKPOINTER_ARRAY_SIZE entries NOTE: The default value for the
//     init_bp_array_size is BLOCKPOINTER_ARRAY_SIZE. The default value for the
//     initi_block_size is BLOCK_SIZE.
//
//
//
...
//
//   void reserve()
//    Postcondition: Increases the capacity of the deque by 20x BLOCK_SIZE
//
...
public:
  ...
  static const size_t BLOCKPOINTER_ARRAY_SIZE =
      5; // Number of entries in the array of block pointers (The minimum
         // acceptable value is 2)

  typedef std::size_t size_type;
  typedef Item value_type; // The type of items stored in the deque

  typedef deque_iterator<Item>
      iterator; // A typedef to simplify the use of iterator class
...
private:
  ...
  value_type **block_pointers_end; // A pointer to the final entry in the array
                                   // of block pointers

  value_type **first_bp; // A pointer to the entry of the array of block
                         // pointers which points to the first data block
  value_type **last_bp; // A pointer to the entry of the array of block pointers
                        // which points to the last data block
...
deque<Item>::deque(size_type init_bp_array_size, size_type init_block_size) {
  ...
  // Create an an array of block pointers and assign its address to
  // "block_pointers"
...
template <class Item> void deque<Item>::operator=(const deque<Item> &source) {
  ...
  for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size;
       ++bp_array_index) {
    // If an entry of source.block_pointers is NULL, that entry should be NULL
    ...
    if (source.block_pointers[bp_array_index] == NULL) {
      ...
      for (size_type block_item_index = 0; block_item_index < block_size;
           ++block_item_index) {
        // STUDENT WORK...
        if (source.block_pointers[bp_array_index] + block_item_index ==
            source.front_ptr)
          front_ptr = block_pointers[bp_array_index] + block_item_index;
        if (source.block_pointers[bp_array_index] + block_item_index ==
            source.back_ptr)
          back_ptr = block_pointers[bp_array_index] + block_item_index;
        *(block_pointers[bp_array_index] + block_item_index) =
            *(source.block_pointers[bp_array_index] + block_item_index);
      }
    }
  }
}
...
template <class Item> void deque<Item>::clear() {
...
template <class Item> void deque<Item>::reserve() {
...
template <class Item> void deque<Item>::push_front(const value_type &entry) {
  // Only the array of block pointers exists (and no data block exists)
  if (first_bp == NULL) {
    assert(bp_array_size > 1);
    size_t bp_mid = floor(
        bp_array_size / 2); // Get the mid point of the array of block pointers
...
template <class Item> void deque<Item>::push_back(const value_type &entry) {
  // Only the array of block pointers exists (and no data block exists)
  if (last_bp == NULL) {
    assert(bp_array_size > 1);
    size_t bp_mid = floor(
...
template <class Item> typename deque<Item>::value_type deque<Item>::back() {
  assert(!isEmpty());
  return *back_ptr;
}

template <class Item> typename deque<Item>::value_type deque<Item>::front() {
  assert(!isEmpty());
  return *front_ptr;
}
...
template <class Item> typename deque<Item>::iterator deque<Item>::begin() {
  ...
  return iterator(block_pointers, block_pointers_end, first_bp, last_bp,
                  front_ptr, back_ptr, bp_array_size, block_size, tmp_cursor,
                  tmp_current_boundary, tmp_current_block_pointer);
}

// Constructs an itertor which points to the past the
// last element of the deque
template <class Item> typename deque<Item>::iterator deque<Item>::end() {
  value_type *tmp_cursor = NULL;
  value_type *tmp_current_boundary = NULL;
  value_type **tmp_current_block_pointer = NULL;
