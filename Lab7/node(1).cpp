...
namespace coen79_lab7
{
  ...
  void list_tail_insert(node*& tail, const std::string &newName, const float &newPrice) {
    if(tail== NULL) //check if empty
      return;
    node *insert_ptr = new node(newName, newPrice, NULL); //create new node
    tail->setLink(insert_ptr); //set tail link to new node
    tail = tail->getLink();   
  }
  ...
  void list_clear(node*& head) {
    if (head == NULL) //check if empty list
      return;
    while (head != NULL){ //while not empty, delete head
  ...
  void list_head_remove(node*& head) {
    ...
    node *delete_ptr = head; //create temporary pointer
    head = head->getLink(); //point to head
    delete delete_ptr; //delete temorprary pointer
  }
